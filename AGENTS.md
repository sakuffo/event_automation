...
- `models.py` — Pydantic `EventRecord` (+ `content_hash()` for change detection, bookkeeping fields `notion_page_id`/`wix_event_id`/`status`/`synced_hash`, pull-only sales fields `tickets_sold`/`tickets_sold_by_type`/`revenue`); `TicketSpec`/`parse_tickets` for `;`-separated multi-ticket fields
...
- **One row per recurring series**: secondary recurring occurrences are skipped on pull (`is_secondary_recurring_occurrence`; only `RECURRING_UPCOMING` represents the series). → [full text](docs/INVARIANTS.md#one-row-per-recurring-series)
