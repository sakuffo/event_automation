...
enough), and `Default Instructor` (applied when a row's Instructor is blank;
it lands in the description as "Instructors: …").
...
- **Idea** — a bare placeholder. A linked Template (or a Name matching one)
  ...
  carries Default Start/End Times. `enrich` copies the template's name into a
