...
| Status | select | humans + sync | `Idea` → `Draft` → `Ready` → `Published`; `Cancel` → `Cancelled`; `Delete` → `Removed`; plus `Error` and `Skip` |
...
| Capacity | number | enrich or humans | |
| Ticket Price | number | enrich (pricing table) or humans | Single-ticket price |
| Ticket Names / Ticket Prices / Ticket Capacities | rich text | humans | Semicolon-separated for multi-ticket events (`Regular; Student`) |
...
| Synced Hash | rich text | **code only** | Hash of the last-pushed payload; drives change detection |
...
`Tagline`, `Description`, `Image URL`, optional `Price Override` and
`Default Capacity`.
...
```
Idea ──enrich──▶ Draft ──human review, flip to──▶ Ready ──sync──▶ Published
                                                            │         │
                                                        (failure)   flip to Cancel ──sync──▶ Cancelled
                                                            ▼                                   │
                                                          Error       flip to Delete ──sync──▶ Removed
                                                                       (from any state)
```

- **Idea** — a bare placeholder. Only a Name (matching a catalog template) and
  a Date are needed; `enrich` fills the rest and promotes it to Draft. If
  required fields are still missing, the row stays put and `Sync Error` says
  what's missing.
...
- **Published** — live on Wix. Edit the row and the next `sync` detects the
  change via `Synced Hash` and patches Wix — no status flipping needed.
...
- **Error** — something failed; the reason is in `Sync Error`. Fix and set
  back to Ready (or edit a Published row and re-sync).
...
From then on, clicking `New` gives a row where you only type the Name, link
the Template, and pick the Date. If someone skips the template (or a value),
the pipeline still fills every blank from Settings at `enrich` time — and as
a safety net at `sync` time for rows flipped straight to `Ready`.
