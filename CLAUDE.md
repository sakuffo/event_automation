...
- `runtime.py` — `SyncRuntime` lazily holds `WixClient`, `NotionStore`, and the Google Drive client (the Google SDK import lives inside `get_drive_service` so Notion-only commands never pay it); image download/upload caches; `last_image_failure` bookkeeping
